...
let isTimerPaused = false
 ...
 class CustomRightTimer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      _timer: 0,
  ...
  componentWillUnmount(){
    ...
    AsyncStorage.getItem(KeyUtils.RESET_TIMER).then(value => {
      if (value !== 'true') {
        ...
        AsyncStorage.removeItem(KeyUtils.RIGHT_TIMER_VALUE)
      }
    });
  }
  ...
  componentDidUpdate(prevProps: Readonly<P>, prevState: Readonly<S>, snapshot: SS) {
    ...
    if (appState && appState != prevProps.appState) {
      if (appState === 'background') {
        ...
        AsyncStorage.getItem(KeyUtils.IS_TIME_ACTIVE_BFnP_RIGHT).then(value => {
          if (value === 'true') {
            AsyncStorage.getItem(KeyUtils.RIGHT_TIMER_VALUE).then(startTime => {
              AsyncStorage.getItem(KeyUtils.BACKGROUND_TIME_STAMP).then(backgroundTime => {
                ...
                AsyncStorage.setItem(KeyUtils.RIGHT_TIMER_VALUE, (dd).toString())
  ...
  async init(){
    const {stopwatchStartStatus, isCallingFromBaby, secondsDifference,isRightPress} = this.props
    let resetValue=await AsyncStorage.getItem(KeyUtils.RESET_TIMER);
    if(resetValue==='true'){
      this.handleReset()
    }else{
      let timer =await AsyncStorage.getItem(KeyUtils.RIGHT_TIMER_VALUE);
      if(timer && timer !==''){
    ...
    AsyncStorage.getItem(KeyUtils.IS_TIME_ACTIVE_BFnP_RIGHT).then(value => {
      if (value === 'true') {
        if (secondsDifference !== -1) {
          ...
          this.setState({_timer: dd}, () => {
            if (secondsDifference) {
              AsyncStorage.setItem(KeyUtils.RIGHT_TIMER_VALUE, secondsDifference.toString())
              this.handleStart();
            }
          });
        }
        ...
        if (this.props.isPausedFromSiri) {
          ...
          this.setState({ _timer: dd }, () => {
          AsyncStorage.setItem(KeyUtils.RIGHT_TIMER_VALUE, this.props.secondsDifference.toString())
          })}
        else{
          this.setState({ _timer: 0 }, () => {
           AsyncStorage.setItem(KeyUtils.RIGHT_TIMER_VALUE, '0')
          })}}
          this.formatTime()
      } else {
        if (!isCallingFromBaby && isRightPress) {
          this.startTimer()
        }
      }
    });
  ...
  handleStart(){
    ...
    const {stopwatchStartStatus,changeValue, isEditable, handleStartLeftTimer, callChild} = this.props

    let mObj = {stopwatchSelected: !stopwatchStartStatus};
    if (stopwatchStartStatus) {
      callChild!==0 && callChild!==2 && handleStartLeftTimer()
      ...
      this._increment.current = setInterval(() => {
        ...
        this.setState({
          ...
          AsyncStorage.setItem(KeyUtils.START_TIMESTAMP_BFnP_RIGHT, Date.now().toString())
          AsyncStorage.setItem(KeyUtils.RIGHT_TIMER_VALUE, this.state._timer.toString())
          this.formatTime()
        })
      }, 1000)
    } else {
  ...
  handlePause(){
    const {handlePauseLeftTimer} = this.props
    isTimerPaused = true
    AsyncStorage.setItem(KeyUtils.IS_TIME_ACTIVE_BFnP_RIGHT, 'pause')
    AsyncStorage.setItem(KeyUtils.IS_RIGHT_TIMER_STARTED, 'pause')
    clearInterval(this._increment.current);
  ...
  handleReset(){
    ...
    AsyncStorage.setItem(KeyUtils.PAUSE_RIGHT_TIMER, 'false');
  }
  ...
  onChangeMin(value) {
    const {hour, min, sec} = this.state
    const {leftTimerInSec,leftTimerInMin}=this.props
    this.handlePause()
    let rightTotalTimeInSec=getTotalMinHoursInSec(hour,value==''?0:value,parseInt(sec))
    //let leftTotalTimeInSec=getTotalMin(leftTimerInMin,leftTimerInSec)
   // let totalTime=leftTotalTimeInSec+rightTotalTimeInSec

    if(value.length>1){
      this.secInput.focus()
    }
    if (value <= 60 && rightTotalTimeInSec<=6000) {
...
/*    if(value.length>1){
  ...
  onChangeSec(value) {
    ...
    const {leftTimerInSec,leftTimerInMin}=this.props
    this.handlePause()
    let rightTotalTimeInSec=getTotalMin(min,value==''?0:value)
    let leftTotalTimeInSec=getTotalMin(leftTimerInMin,leftTimerInSec)
    let totalTime=leftTotalTimeInSec+rightTotalTimeInSec
    if(value==''){
      this.minInput.focus()
  ...
  renderTimer=()=> {
    ...
    return (
      <TouchableOpacity accessible={true} accessibilityLabel={I18n.t('breastfeeding_pump.right_timer')}  onPress={() => {
          this.startTimer()
      ...
      >
        <View
          ...
          {icon ? stopwatchStartStatus ? icon : <Text
  ...
  renderEditableTimer() {
    ...
    return (
      <View style={[styles.StopwatchWrapper,{right:moderateScale(25)}]}>
        {this.renderTimer()}
        <View style={options.container}>
          {timerTypeStyle === 3 ? <View style={[styles.timeBackground, {flexDirection: 'row', alignItems: 'center',height:48,}]}>
              ...
              />
              <Text accessibilityLabel={I18n.t("breastfeeding_pump.right_hrs_separator")} accessible={true} maxFontSizeMultiplier={1.7} style={[styles.textInputStyles, {marginBottom: 5,color:Colors.rgb_000000}]}>{":"}</Text>
              <CustomTextInput
                maxLength={2}
                keyboardType={'numeric'}
                textStyles={[styles.textInputStylesCustom,{color:Colors.rgb_000000}]}
                ...
                contextMenuHidden={true}
              />
              <Text accessibilityLabel={I18n.t("breastfeeding_pump.right_min_separator")} accessible={true} maxFontSizeMultiplier={1.7} style={[styles.textInputStyles, {marginBottom: 5,color:Colors.rgb_000000}]}>{":"}</Text>
              <CustomTextInput
                maxLength={2}
                keyboardType={'numeric'}
                textStyles={[styles.textInputStylesCustom,{color:Colors.rgb_000000}]}
            ...
            <View style={[styles.timeBackground, {flexDirection: 'row', alignItems: 'center'}]}>
              <CustomTextInput
                maxLength={2}
                keyboardType={'numeric'}
                textStyles={[styles.textInputRightStylesCustom,{color:Colors.rgb_000000}]}
                value={min}
                placeholder={'00'}
                ...
                inputRef={(input)=>{ this.minInput = input }}
              />
              <Text maxFontSizeMultiplier={1.7} style={[styles.textInputStyles, {marginBottom: 5,color:Colors.rgb_000000}]}>{":"}</Text>
              <CustomTextInput
                maxLength={2}
                keyboardType={'numeric'}
                textStyles={[styles.textInputLeftStylesCustom,{color:Colors.rgb_000000}]}
                value={sec}
                isSecChange={true}
                placeholder={'00'}
                onFocus={(index, value) => this.onFocusValue()}
...
export default connect(mapStateToProps, null,null,{forwardRef:'true'})(CustomRightTimer)
